...
const Pdfs = () => {
  ...
  const handleCourseClick = async (
    ...
    try {
      ...
      // Get topic IDs to fetch their PDFs
      const topicIds = classTopics.map((topic: any) => topic.id) || [];

      if (topicIds.length > 0) {
        // Get PDFs for these specific topics using context
        const allPDFs = getResourcesByType("pdf");
        const topicPDFs = allPDFs.filter(
          (pdf: any) => pdf.topic_id && topicIds.includes(pdf.topic_id)
        );

        setPdfs(topicPDFs || []);
        setTopicList(true);
        console.log("PDFs found:", topicPDFs);
      } else {
        setPdfs([]); // No topics means no PDFs
      }

      // Open drawer to show topics and PDFs
      onOpen();
    } catch (error) {
