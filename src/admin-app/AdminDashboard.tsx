...
const AdminDashboard = () => {
  ...
  const navItems: { key: Tab | "cms"; label: string; sub: string; icon: React.ElementType; path?: string }[] = [
    ...
    { key: "cms" as any, label: "CMS Workspace", sub: "Manage topics & resources", icon: FiBookOpen, path: "/admin/content-management" },
        ...
        <Stack gap={1} p={4}>
          {navItems.map((item) => (
            <Box
              ...
              onClick={() => {
                if (item.path) {
                  // If you are using next/navigation or react-router-dom, use router.push(item.path) here
